...
for i in range(0,new_len):
    word += str(int(frame_times[i]*1000))
    word += "a"
word += "b"
for i in range(0,new_len):
...
temp = 0
while True:
    if (temp*60+60)<=len(word):
        print(word[temp*60:temp*60+60])
        ser.write(bytes(word[temp*60:temp*60+60],'utf-8'))
    else:
        ser.write(bytes(word[temp*60:],'utf-8'))
        print(word[temp*60:])
        break
    time.sleep(0.3)
    temp += 1
