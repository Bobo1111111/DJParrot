...
def beat_strength_graph(y,sr,beat_frames,onset_data,type = 'time',step = 30, label='Hz'):
    ...
    if type == 'time':
        ...
        plt.plot(beat_times,onset,label=label)
...
def play_drum(filename,target,stoptime=60):
    pygame.mixer.init()
    ...
    i = 0
    while i < len(target):
        if (time.time()-start) > target[i]-0.1:
